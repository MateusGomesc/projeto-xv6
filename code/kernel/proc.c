...
scheduler(void)
{
  struct proc *p; // Variavel para guardar processo
  struct cpu *c = mycpu(); // Variavel com representacao do processador

  // Atribui 0 no proc da cpu pra falar que nao tem nenhum processo nela
  c->proc = 0;
  for(;;){
    intr_on();
    ...
    // 1ª passada: descobre a maior prioridade entre os prontos
    for(p = proc; p < &proc[NPROC]; p++){
      acquire(&p->lock);
    ...
    // 2ª passada: round robin só entre quem tem essa prioridade
    for(p = proc; p < &proc[NPROC]; p++){
      acquire(&p->lock);
      if(p->state == RUNNABLE && p->priority == highest){
        p->state = RUNNING;
...
get_runnable_procs(uint64 user_array)
{
  ...
  for(p = proc; p < &proc[NPROC]; p++){
    ...
    if(p->state == RUNNABLE){
      // Copying data for a temp struct
